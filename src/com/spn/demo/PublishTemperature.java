...
public class PublishTemperature extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
             ...
             System.out.println("current folder"  + current);
            conf = new Configs();
        } catch (IOException e) {
...
//                       Integer payload = 1;
            Integer payload = new Integer(request.getParameter("temperature"));
            Event e = new Event();
            e.setClientId(this.conf.getProperty("spnClientID"));
            e.setPayload(this.encrypt(payload.toString().getBytes()));
     ...
     * new File(savePath); if (!fileSaveDir.exists()) { fileSaveDir.mkdir(); }
     * for (Part part : request.getParts()) { String fileName =
     * extractFileName(part); part.write(savePath + File.separator + fileName);
     * } }
*
     */
